# ============================================================
# plot.py (MDE) -> genera el PDF con las gráficas del mejor modelo
# ============================================================
import os
import json
import numpy as np
...
DESCRIPCIONES = {
    ...
    "embedding_dim": "Dimensión de embedding m (largo de cada patrón de dispersión)",
    "classes": "Nº de clases de amplitud c del mapeo con la normal",
    "delay": "Retardo entre puntos del patrón",
    "test_size": "Proporción de la señal usada como test",
    "validation_size": (
        "Proporción de la señal usada para seleccionar hiperparámetros"
    ),
...
def generar_pdf(
    ...
    with open(os.path.join(models_dir, "parametros.json"), encoding="utf-8") as f:
        params = json.load(f)
    pref = "mde"
    pdf_path = os.path.join(reports_dir, "graficas_mejor_modelo_MDE.pdf")
    with PdfPages(pdf_path) as pdf:
        # Página 1: lista de parámetros (nombre, valor y descripción)
        fig = plt.figure(figsize=(8.5, 11))
        fig.suptitle("Multiscale Dispersion Entropy: parámetros del mejor modelo", fontsize=13)
        y0 = 0.93
        ...
        plt.title("Curvas de convergencia MDE: pérdida normal y penalizada (mGD)"); plt.xlabel("Épocas"); plt.ylabel("Pérdida")
        ...
        ax.set_ylim(0, 1.1)
        ax.set_ylabel("F1-score"); ax.set_title("F-scores MDE (train / test)"); ax.legend()
        fig.tight_layout()
